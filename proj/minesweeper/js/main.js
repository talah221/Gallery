...
function cellMarked(i, j) {
    if (gBoard[i][j].isShown && !gBoard[i][j].isMine && !gBoard[i][j.isMarked]) return;
...
function expandShown(cellI, cellJ) {
    ...
    for (var i = cellI - 1; i <= cellI + 1; i++) {
        for (var j = cellJ - 1; j <= cellJ + 1; j++) {
            if (checkIfInBoard(gBoard, i, j) && !gBoard[i][j].isMine && !gBoard[i][j].isMarked) gBoard[i][j].isShown = true;
            if (checkIfInBoard(gBoard, i, j)&&gBoard[i][j].minesAroundCount===0){
                var cellToOpen=gBoard[i][j];
                var cellLoc={i,j}
                empties.push(cellLoc)
    ...
    if(empties.length){
        ...
        for (var k = 0; k < empties.length; k++) {
            expandShown(empties[k].i,empties[k].j)
    }
